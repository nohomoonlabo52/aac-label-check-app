...
// ★★★ CORS対応のためのライブラリをインポート ★★★
const cors = require('cors')({origin: true});

// Gemini APIキーをFirebaseの環境変数から安全に取得
// ※注: functions.config()は非推奨ですが、v1関数ではまだ動作します
...
exports.analyzeLabel = functions.https.onRequest((req, res) => {
  ...
  cors(req, res, async () => {
    ...
    if (!imageData) {
      ...
      res.status(400).send({ error: { message: "画像データが見つかりません。" } });
      return;
    }
    ...
    try {
      ...
      // onCallと異なり、結果はres.send()で返す
      res.status(200).send({ data: jsonData });

    } catch (error) {
      console.error("AIの解析中にエラーが発生しました:", error);
      // onCallと異なり、エラーはres.status().send()で返す
      res.status(500).send({ error: { message: "AIの解析中にエラーが発生しました。" } });
    }
  });
});